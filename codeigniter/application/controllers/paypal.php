...
class Paypal extends CI_Controller {
    ...
    public function cancelPaypalRequest(){
        echo "Use has cancelled the payment..";
    }

    public function paypalReturnResponse() {
        //echo 'inside paypalResponse';
		...
		//die("data posted");
            ...
            $padata = '&METHOD=SetExpressCheckout' .
                    ...
                    '&LOGOIMG='.base_url_css.'images/mydochub-logo.png' . //site logo
            ...
            if ("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
               ...
               // echo 'Your Transaction ID : ' . urldecode($httpParsedResponseAr["PAYMENTINFO_0_TRANSACTIONID"]);
                ...
                if ('Completed' == $httpParsedResponseAr["PAYMENTINFO_0_PAYMENTSTATUS"]) {
                     $this->session->set_userdata('paypal_msg','<div style="color:green">Payment Received! Credit has been updated in your account!</div>');
                } elseif ('Pending' == $httpParsedResponseAr["PAYMENTINFO_0_PAYMENTSTATUS"]) {
                     $this->session->set_userdata('paypal_msg','<div style="color:red">Transaction Complete, but payment is still pending! ' .
...
//print_r($httpParsedResponseAr);
                ...
                if ("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
                    ...
                    $this->session->set_userdata('paypal_msg', '<div style="color:red"><b>GetTransactionDetails failed:</b>' . urldecode($httpParsedResponseAr["L_LONGMESSAGE0"]) . '</div>');
                   // echo '<pre>';
                    //print_r($httpParsedResponseAr);
                   // echo '</pre>';
				   	 redirect(base_url()."dashboardProfessional");
                ...
                $this->session->set_userdata('paypal_msg','<div style="color:red"><b>Error : </b>' . urldecode($httpParsedResponseAr["L_LONGMESSAGE0"]) . '</div>');
               // echo '<pre>';
               // print_r($httpParsedResponseAr);
               // echo '</pre>';
				   	 redirect(base_url()."dashboardProfessional");
